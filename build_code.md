...
The following notes describe the steps to build **ALC_ART** using the [**CMake**](https://cmake.org) platform and Fortran compilers. In addition, we show how to run the testing infrastructure.
...
## Building the code
We assume the code has been downloaded to folder ***alc_art*** at the location */home/username/codes*, in the remote machine *wherever* of the account *"username"*. Please refer to file [README.md](./README.md) for instructions to download the code.

### Building manually
For manual compilation with CMake, the user must create a folder where to compile the code. It is good practise to name this folder using the word *"build"* together with any other specification that indicates the type of compilation. For example, if the user aims to build ALC_ART using the GNU-Fortran compiler in Debugging mode, the folder could be named *"build-gnu-debug"*:
```sh
username@wherever:/home/username/codes/alc_art$ mkdir build-gnu-debug
username@wherever:/home/username/codes/alc_art$ cd build-gnu-debug
username@wherever:/home/username/codes/alc_art/build-gnu-debug$ FC=gfortran cmake ../ -DCMAKE_BUILD_TYPE=Debug
```
...
```sh
username@wherever:/home/username/codes/alc_art/build-gnu-debug$ make
```
If compilation is successful, the executable *alc_art.x* will have been generated inside folder ***bin***. Extra implemented options are listed below:
...
```sh
username@wherever:/home/username/codes/alc_art$ sh tools/gnu-build-debug.sh
```
...
```sh
"-g -Wextra -Wuse-without-only -frecord-gcc-switches -O0 -std=gnu -pedantic -fbacktrace -fcheck=all -finit-integer=2147483647 -finit-real=snan -finit-logical=true -finit-character=42 -ffpe-trap=invalid,zero,overflow -fdump-core -fstack-protector-all -Wall -pipe"
```
...
To have access to the testing infrastructure for **ALC_ART**, the user must add *-DWITH_TESTING=ON* when building with cmake. Based on the example above to building-only the code, the user must execute the following commands
```sh
username@wherever:/home/username/codes/alc_art$ mkdir test-gnu-debug
username@wherever:/home/username/codes/alc_art$ cd test-gnu-debug
username@wherever:/home/username/codes/alc_art/test-gnu-debug$ FC=gfortran cmake ../ -DCMAKE_BUILD_TYPE=Debug -DWITH_TESTING=ON
```
...
```sh
username@wherever:/home/username/codes/alc_art/test-gnu-debug$ ctest --output-on-failure -R test10
Test project home/username/codes/alc_art/test-gnu-debug
    Start 10: test10
...
SUCCESS !!! OCF_NONREACTIVE_ALL passed the test
SUCCESS !!! OCF_NONREACTIVE_AVG passed the test
...
Errors while running CTest
```
which shows there was a problem with the RDF and OUTPUT files. If, in contrast, the test fails to run, one gets the following:
```sh
username@wherever:/home/username/codes/alc_art/test-gnu-debug$ ctest --output-on-failure -R test10
Test project home/username/codes/alc_art/test-gnu-debug
    Start 10: test10
...
To execute this script, the user must proceed as follows:
```sh
username@wherever:/home/username/codes/alc_art$ sh tools/gnu-test-debug.sh
```
