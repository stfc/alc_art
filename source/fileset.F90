...
Module fileset
  ...
  Integer(Kind=wi), Parameter, Public :: FILE_OCF_NONREACTIVE_ALL=8
  ! Orientational correlation function OCF (averaged)
  Integer(Kind=wi), Parameter, Public :: FILE_OCF_NONREACTIVE_AVG=9
  ...
  Integer(Kind=wi), Parameter, Public :: FILE_OCF_REACTIVE_ALL=23
  ! Orientational correlation function OCF (averaged)
  Integer(Kind=wi), Parameter, Public :: FILE_OCF_REACTIVE_AVG=24
...
Contains
  ...
  Subroutine set_names_files(files)
    ...
    set_names(FILE_OCF_NONREACTIVE_ALL) = "OCF_NONREACTIVE_ALL"
    set_names(FILE_OCF_NONREACTIVE_AVG) = "OCF_NONREACTIVE_AVG"
    ...
    set_names(FILE_OCF_REACTIVE_ALL)  = "OCF_REACTIVE_ALL"
    set_names(FILE_OCF_REACTIVE_AVG)  = "OCF_REACTIVE_AVG"
  ...
  Subroutine print_header_out(files)
    ...
    Character(Len=128)          :: header(15)

    Write (header(1), fmt1)   Repeat("#", 74)
    Write (header(2), fmt2)  "#                      WELCOME TO ", Trim(code_name),  Repeat(" ", 32)//"#"
    Write (header(3), fmt1)  "#  An ALC software designed and developed for the Analysis of Reactive   #"
    Write (header(4), fmt1)  "#  Trajectories from Molecular Dynamics Simulations                      #"
    Write (header(5), fmt1)  "#                                                                        #"
    Write (header(6), fmt3)  "#  version:  ", Trim(code_VERSION), Repeat(' ',57),                     "#"
    Write (header(7), fmt3)  "#  release:  ", Trim(date_RELEASE), Repeat(' ',51),                     "#"
    Write (header(8), fmt1)  "#                                                                        #"
    Write (header(9), fmt1)  "#  Copyright:  2026  Ada Lovelace Centre (ALC)                           #"
    Write (header(10), fmt1) "#              Scientific Computing Department (SCD)                     #"
    Write (header(11), fmt1) "#              Science and Technology Facilities Councils (STFC)         #"
    Write (header(12), fmt1) "#                                                                        #"
    Write (header(13), fmt1) "#  Author:     Ivan Scivetti (SCD/STFC)                                  #"
    Write (header(14), fmt1) "#                                                                        #"
    Write (header(15), fmt1)  Repeat("#", 74)
    Call info(header, 15)
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Write (appex(5), fmt2)  "#  Thanks for using ", Trim(code_name), Repeat(" ", 10)//"#"
    Write (appex(6), fmt1)  "#                                    #" 
