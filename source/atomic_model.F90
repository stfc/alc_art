...
Module atomic_model 
  ...
  ! Type for the  &reference_composition
  Type :: type_reference_composition
  ...
  Type :: reactive_type 
    ...
    Type(environment)  :: search_envr 
    Type(bonding)      :: bonds
    Type(in_string)    :: type
  End Type 

  ! Type to describe the definition of the nonreactive species
  Type, Public :: nonreactive_type
    Type(in_string)       :: invoke
    Type(in_string)       :: name
  ...
  Type, Public :: model_type
    ...
    Type(in_logic), Public                :: reactive_chemistry
    ! Extra bonds    
    Type(extra_bonding), Public           :: extra_bonds
    ! new chemistry
    Type(reactive_type), Public           :: reactive_species
    ! Number of extra species found
    Integer(Kind=wi), Public              :: number_species_found
    ! Type for those components defined in reference_composition
    Type(type_reference_composition), Public  ::  reference_composition 
    ! Format of input trajectory 
    Type(in_string), Public               :: geometry_format
    ! Input model 
    Type(config_type), Public             :: config 
    ! definition of nonreactive species to be monitored 
    Type(nonreactive_type), Public        :: nonreactive_species
    ! Tracked species
   ...
   Contains
     Private
       ...
       Procedure, Public  :: init_reference_composition   => allocate_reference_composition_arrays
       Final              :: cleanup
  End Type model_type
  ...
  Public :: compute_distance_pbc, reference_tag_reactivity
  
Contains

  Subroutine allocate_reference_composition_arrays(T)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    Allocate(T%reference_composition%tag(T%reference_composition%atomic_species),       Stat=fail(1))
    Allocate(T%reference_composition%N0(T%reference_composition%atomic_species),        Stat=fail(2))
    Allocate(T%reference_composition%element(T%reference_composition%atomic_species),   Stat=fail(3))

    If (Any(fail > 0)) Then
      Write (message,'(1x,1a)') '***ERROR: Allocation problems for the input composition&
                                & (subroutine allocate_reference_composition_arrays).'
      Call error_stop(message)
    End If

  End Subroutine allocate_reference_composition_arrays
  ...
  Subroutine cleanup(T)
    ...
    If (Allocated(T%reference_composition%tag)) Then
      Deallocate(T%reference_composition%tag)
    End If 

    If (Allocated(T%reference_composition%N0)) Then
      Deallocate(T%reference_composition%N0)
    End If 

    If (Allocated(T%reference_composition%element)) Then
      Deallocate(T%reference_composition%element)
    End If 
  ...
  Subroutine atomistic_model(model_data, frame)
    ...
    ! - identify the set of atoms that belong to the nonreactive species
    !   to be monitored
    !
    ! author    - i.scivetti May 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If(model_data%reactive_chemistry%stat) Then 
      ...
      Else
         Call reactive_species_changes(model_data, frame)   
      End If
    ...
    If (model_data%nonreactive_species%invoke%fread) Then
      Call check_non_reactive_species(model_data, frame)
    End If

  End Subroutine atomistic_model

  Subroutine check_non_reactive_species(model_data, frame)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Identify molecular, nonreactive species to be
    ! monitored along the trajectory
    !
    ! author    - i.scivetti May 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(model_type),  Intent(InOut) :: model_data
    ...
    Do j = 1, model_data%config%Nmax_species
      ...
      If(model_data%config%atom(k)%tag==model_data%nonreactive_species%reference_tag%type) Then
  ...
  End Subroutine check_non_reactive_species
  
  Subroutine reference_tag_reactivity(model_data, tag_reacts)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Subroutine to check if the reference tag defined in &nonreactive_species
    ! can become part of a reative site
    !
    ! author    - i.scivetti July 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(model_type),  Intent(In   ) :: model_data
    Logical,           Intent(  Out) :: tag_reacts
  
    Integer(Kind=wi) :: j
    
    tag_reacts=.False.
    
    Do j= 1, model_data%reactive_species%search_envr%N0_incl
      If (Trim(model_data%reactive_species%search_envr%tg_incl(j)) == & 
         Trim(model_data%nonreactive_species%reference_tag%type) ) Then
        tag_reacts=.True. 
      End If
    End Do
  
  End Subroutine reference_tag_reactivity
  ...
  Subroutine identify_species(model_data, kin, jin, frame)
    ...
    bond_cutoff=model_data%nonreactive_species%bond_cutoff%value    
    ...
    Do While (loop)
      ...
      Do knn=1, num_nn
        ...
        Do k=1, model_data%config%num_atoms
          loop_sp=.True.
          m=1
          Do While (m <= model_data%nonreactive_species%num_components .And. loop_sp)
            If ((model_data%config%atom(k)%element==model_data%nonreactive_species%element(m)) .And. k/=ka .And.&
                   ...
                   If (is_nn) Then    
                     ...
                     If (nacc==max_neighbours+1) Then
                        ...
                        Write (messages(2),'(a)') ' It is likely the species represents does not represent a molecule!&
                                                 & Please review the definition of "&selected_nonreactive_species".'
                     ...
                     il=1
                     is_list=.True.
                     Do While (il <= model_data%nonreactive_species%atoms_per_species .And. is_list) 
        ...
        If (.Not. model_data%config%atom(ka)%identified) Then
          ...
          num_tot=num_tot+1
          If (num_tot+nacc == model_data%nonreactive_species%atoms_per_species) Then
            Do knni= 1, nacc
              model_data%config%atom(list_nn_next(knni))%identified=.True.
            End Do
            loop=.False.
            num_tot=model_data%nonreactive_species%atoms_per_species
          End If
        End If
      End Do
      num_nn=nacc  
    ...
    If (model_data%nonreactive_species%atoms_per_species/=num_tot) Then
      Write (messages(1),'(a,i4,3a,i6)') '***ERROR: Trouble to identify unit ', jin, ' of the nonreactive species "',&
                               & Trim(model_data%nonreactive_species%name%type), '" for frame: ', frame
      ...
      Write (messages(4),'(a,f4.2,a)')  ' 2) the value for the bond cutoff (', bond_cutoff, &
                                    & ' Angstrom) as specified in &selected_nonreactive_species&
      ...
      Write (messages(6),'(a)') ' 4) wrong definition of the nonreactive species.&
      ...
      If (model_data%reactive_chemistry%stat) Then
         Write (messages(1),'(a)') '************************************************************************************'
         Write (messages(2),'(a)') '*** IMPORTANT: also check the settings of the &reactive_species block, particularly '
  ...
  Subroutine obtain_maximum_number_species(model_data)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Obtain the maximum amount of species
    !
    ! author    - i.scivetti May 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(model_type), Intent(InOut) :: model_data
    ...
    Do j = 1, model_data%config%num_atoms
      If(model_data%config%atom(j)%tag_0==model_data%nonreactive_species%reference_tag%type) Then
  ...
  Subroutine tracking_chemistry(model_data, frame)
   ...
   Character(Len=256) :: messages(2)
    ...
    If (frame == 1) Then
      Do i = 1, model_data%reactive_species%N0%value
        model_data%track_chem(i)%indx=model_data%reactive_species%indx_new(i)
      End Do
    Else        
       Do i = 1, model_data%reactive_species%N0%value
        dist_min=Huge(1.0_wp)
        Do  j = 1, model_data%reactive_species%N0%value
          indx_new  = model_data%reactive_species%indx_new(j)
    ...
    Do i=1, model_data%reactive_species%N0%value-1
      Do j=i+1, model_data%reactive_species%N0%value
    ...
    End Do
    model_data%reactive_species%indx_prev = model_data%reactive_species%indx_new
  ...
  Subroutine identify_initial_chemistry(model_data, frame)
    ...
    ! Check for bonds according to the criteria set in the &extra_reactive_bonds block 
    If (model_data%extra_bonds%invoke%fread) Then
      Call initial_extra_bonding(model_data, 1)
    End If

    ! Find those sites with the type and amount of bonds as specified in the &reactive_species block
    If (model_data%number_species_found /= model_data%reactive_species%N0%value) Then
      i=1
      ...
      Do i=1, model_data%config%num_atoms
        If (model_data%config%atom(i)%Nbonds == model_data%reactive_species%bonds%N0%value) Then
           model_data%number_species_found = model_data%number_species_found+1 
           model_data%reactive_species%indx_new(model_data%number_species_found) = i
           model_data%config%atom(i)%tag=Trim(model_data%config%atom(i)%tag)//'*'
           Do k=1, model_data%reactive_species%bonds%N0%value
  ...
  Subroutine error_chemistry(model_data, condition) 
    ...
    If (condition=='failed_initial_counting') Then
      ...
      Else
        ...
        Call info (messages, 1)
        If (model_data%extra_bonds%invoke%fread) Then
          Write (messages(1), '(1x,a)') '- the information provided in the "&extra_reactive_bonds" is not correct (wrong& 
        ...
        Else
          Write (messages(1), '(1x,a)') '- the system could also form bonds with other species. The user&
                                       & should consider using the "&extra_reactive_bonds" block'
          Call info (messages, 1)
        End If        
      End If
      Write (messages(1), '(1x,a)') '- the value assigned to "total_number" in the  "&reactive_species" block is incorrect'
    ...
    Else If (condition=='failed_counting') Then
      If (model_data%number_species_found > model_data%reactive_species%N0%value) Then
        Write (messages(1), '(1x,a)') '- the "cutoff" directive of the "&search_environment" sub-block is too large'
      Else  
        Write (messages(1), '(1x,a)') '- the "cutoff" directive of the "&search_environment" sub-block is too low'
      End If
      Call info (messages, 1)
      If (model_data%reactive_species%search_envr%info_exclude%fread) Then
    ...
    If (model_data%geometry_format%type=='xyz') Then
       Write (messages(1),'(1x,a)') '- vectors defined in the &simulation_cell block do not correspond to the generated trajectory'
       Call info (messages, 1)
    End If

    Write (messages(1), '(1x,a)') '- the atomic configuration is highly unstable (non-equilibrated trajectory)'
    Call info (messages, 1)
    Call error_stop(' ')

  End Subroutine error_chemistry

  Subroutine reactive_species_changes(model_data, frame)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    If (model_data%reactive_species%search_envr%check) Then
      Call check_environment_size(model_data, frame) 
    End If

    ! Initialize arrays
    model_data%reactive_species%search_envr%accum_indxs=0
    ...
    If (model_data%extra_bonds%invoke%fread) Then
      ...
      Do l=1, model_data%reactive_species%N0%value
        match=.False.
        i=model_data%reactive_species%indx_prev(l)
          j=1
    ...
    Do l=1, model_data%reactive_species%N0%value
      If (model_data%extra_bonds%flag(l)) Then
        ...
        ihit=0
        fprev=.True.
        Do While (k2 <= model_data%reactive_species%search_envr%num_nn .And. fprev)
          m=model_data%reactive_species%search_envr%list(k2)
          fenv=.True.
          ...
          If (fenv) Then
            ...
            If (model_data%config%atom(m)%Nbonds == model_data%reactive_species%bonds%N0%value) Then
              Do n1=1, model_data%reactive_species%bonds%N0%value
                n2=model_data%config%atom(m)%bonds(n1)
                !model_data%config%atom(n2)%tag=Trim(model_data%config%atom(n2)%tag)//'*'       
              End Do
              If (k2/=1) Then
                If ( ( .Not. (Any(model_data%reactive_species%search_envr%accum_indxs==m)) ) .And. &
                     ( .Not. (Any(model_data%reactive_species%indx_prev==m)) ) ) then
                  ihit=ihit+1
                  N0_counted=N0_counted+1
                  model_data%reactive_species%search_envr%accum_indxs(N0_counted)=m
                End If
              Else
                If (.Not. (Any(model_data%reactive_species%search_envr%accum_indxs==m)) ) then
                  ihit=ihit+1
                  N0_counted=N0_counted+1
                  model_data%reactive_species%search_envr%accum_indxs(N0_counted)=m
                  fprev=.False.
        ...
        If (ihit == 0) Then
           ...
           Do indx = 1, N0_counted     
             If ( .Not. (model_data%reactive_species%search_envr%accum_indxs(indx)==&
                         model_data%reactive_species%search_envr%list(1))) Then     
               icc=icc+1
             End If
           End Do
           If (icc == N0_counted) Then
             N0_missed=N0_missed+1
             model_data%reactive_species%search_envr%missed(N0_missed)=model_data%reactive_species%search_envr%list(1)
           End If    
        End If  
      End If
    End Do
    ...
    If (model_data%number_species_found < model_data%reactive_species%N0%value) Then
      If (N0_counted/=0) Then
        model_data%reactive_species%search_envr%order_indxs(1)=model_data%reactive_species%search_envr%accum_indxs(1)
        icount=1
        Do j = icount+1, N0_counted
          match=.True. 
          i=1
          Do While (i <= icount .And. match)
             If (model_data%reactive_species%search_envr%accum_indxs(j) ==&
                 model_data%reactive_species%search_envr%order_indxs(i)) Then 
                match=.False.
             End If  
             i=i+1   
          End Do
          If (match) Then
            icount=icount+1 
            model_data%reactive_species%search_envr%order_indxs(icount)=model_data%reactive_species%search_envr%accum_indxs(j)     
          End If        
        End Do
        Do i=1, icount
          model_data%reactive_species%indx_new(i+model_data%number_species_found) =&
          model_data%reactive_species%search_envr%order_indxs(i)
          ! Retag
          ni=model_data%reactive_species%search_envr%order_indxs(i)
    ...
    If (model_data%number_species_found < model_data%reactive_species%N0%value) Then
      If (N0_missed /=0 ) Then
        Do i=1, N0_missed
          model_data%reactive_species%indx_new(i+model_data%number_species_found) =&
          model_data%reactive_species%search_envr%missed(i)
          ! Retag
          ni = model_data%reactive_species%search_envr%missed(i)
          model_data%config%atom(ni)%tag=Trim(model_data%config%atom(ni)%tag)//'*'
    ...
    icount=0
    Do k1=1, model_data%reactive_species%N0%value
      i=model_data%reactive_species%indx_new(k1)
      Do k2= 1,  model_data%config%atom(i)%Nbonds 
        icount=icount+1
        model_data%reactive_species%bonds%list(icount)=model_data%config%atom(i)%bonds(k2)
      End Do
    End Do  
    ...
    Do k1= 1, icount
      Do k2= 1, icount
        If(k1/=k2 .And. (model_data%reactive_species%bonds%list(k1)==model_data%reactive_species%bonds%list(k2))) Then
          match=.True.
        End If        
      End Do
    End Do
  ...
  End Subroutine reactive_species_changes
  ...
  Subroutine find_species_bonds(model_data, i, j)
    ...
    Do While (m <= model_data%reactive_species%search_envr%N0_incl .And. (.Not. match1))
      If(model_data%config%atom(i)%tag==model_data%reactive_species%search_envr%tg_incl(m) .Or.&
         model_data%config%atom(j)%tag==model_data%reactive_species%search_envr%tg_incl(m)) Then
        match1=.True. 
        If((model_data%config%atom(i)%tag==model_data%reactive_species%search_envr%tg_incl(m)) .And. &
           (.Not. model_data%config%atom(i)%identified))Then
          indx=i
          If (((model_data%config%atom(j)%element==model_data%reactive_species%bonds%species%type) .And. &
              (.Not. model_data%config%atom(j)%identified))) Then
            match2=.True.  
          End If
        
        Else If((model_data%config%atom(j)%tag==model_data%reactive_species%search_envr%tg_incl(m)) .And.&
                (.Not. model_data%config%atom(j)%identified)) Then
          indx=j
          If ((model_data%config%atom(i)%element==model_data%reactive_species%bonds%species%type) .And. &
              (.Not. model_data%config%atom(i)%identified)) Then
            match2=.True.  
          End If
    ...
    If (match1 .And. match2) Then
      Call compute_distance_PBC(model_data%config%atom(i)%r, model_data%config%atom(j)%r,&
                              & model_data%config%cell, model_data%config%invcell, dist)
      If (dist < model_data%reactive_species%bonds%cutoff%value ) Then
        model_data%config%atom(indx)%Nbonds=model_data%config%atom(indx)%Nbonds+1 
        N0=model_data%config%atom(indx)%Nbonds
        If(model_data%config%atom(i)%element==model_data%reactive_species%bonds%species%type) Then
          model_data%config%atom(i)%identified=.True.
          model_data%config%atom(j)%bonds(N0)=i
        Else If(model_data%config%atom(j)%element==model_data%reactive_species%bonds%species%type) Then
          model_data%config%atom(j)%identified=.True.
  ...
  Subroutine check_environment_size(model_data, frame)
    ...
    Do l=1, model_data%reactive_species%N0%value
      Call define_environment(model_data, l)
      If (model_data%reactive_species%search_envr%num_nn/=1) Then
        icount=icount+1      
      End If        
    End Do
    ...
    If (icount==0) Then
      ...
      Write (messages(3), '(1x,a)') 'Please review the settings of the &reactive_species block and this particular&
    ...
    model_data%reactive_species%search_envr%check=.False. 
  ...
  Subroutine define_environment(model_data, kindx)
    ...
    icount=0
    model_data%reactive_species%search_envr%num_search=1
    model_data%reactive_species%search_envr%num_nn=1
    i=model_data%reactive_species%indx_prev(kindx)
    model_data%reactive_species%search_envr%list(model_data%reactive_species%search_envr%num_nn)=i
    ...
    Do While (j <= model_data%config%num_atoms)
      If (i/=j) Then
        ...
        If (model_data%reactive_species%search_envr%info_exclude%fread)Then
          m=1
          Do While (m <= model_data%reactive_species%search_envr%N0_excl .And. (.Not. match_self))
            If(model_data%config%atom(i)%tag==model_data%reactive_species%search_envr%tg_excl(m) .And.&
               model_data%config%atom(j)%tag==model_data%reactive_species%search_envr%tg_excl(m)) Then
               match_self=.True.
            End If
            m=m+1
          End Do
        ...
        If (.Not. match_self) Then
          match_i=.False.
          match_j=.False.
          Do k = 1, model_data%reactive_species%search_envr%N0_incl
            If (model_data%config%atom(i)%tag==model_data%reactive_species%search_envr%tg_incl(k)) Then  
              match_i=.True.
            End If
            If (model_data%config%atom(j)%tag==model_data%reactive_species%search_envr%tg_incl(k)) Then  
              match_j=.True.
            End If
          End Do
          If(match_i .And. match_j) Then
            ...
            Else                
              Call compute_distance_PBC(model_data%config%atom(i)%r, model_data%config%atom(j)%r,&
                                    & model_data%config%cell, model_data%config%invcell, dist)
            End If                
            If (dist < model_data%reactive_species%search_envr%cutoff%value) Then
              icount=icount+1
              model_data%reactive_species%search_envr%num_nn=model_data%reactive_species%search_envr%num_nn+1
              If (model_data%reactive_species%search_envr%num_nn > max_components) Then
                Write (message,'(1x,a)') '***ERROR - The number of enviroment neighbours has reached a default maximum.' 
                Call info (message, 1)
                Write (message,'(4x,a)') 'Either review the settings of "&search_environment" or increase the&
                                      & "max_components" parameter in the code and recompile'
                Call info (message, 1)                      
                Call error_stop(' ')
                End If
              model_data%reactive_species%search_envr%list(model_data%reactive_species%search_envr%num_nn)=j
            End If
          End If
        End If
      End If
  ...
  Subroutine initial_extra_bonding(model_data, frame)
    ...
    If (model_data%number_species_found > model_data%reactive_species%N0%value) Then
      Write (messages(1), '(1x,a,i6,a)') '*** ERROR: problems to identify extra bonds (&extra_reactive_bonds sub-block)&
      ...
      Write (messages(3), '(1x,a)') 'Please review the settings of the &extra_reactive_bonds block and this particular&
                                    & configuration in the TRAJECTORY file' 
      Call info (messages, 3)
      Call error_stop(' ')
    End If
    ...
    Do k = 1, model_data%extra_bonds%N0
      If(.Not. match) Then
        ...
        If (match) Then
          If((model_data%config%atom(i)%element==model_data%reactive_species%bonds%species%type) .And. &
             (.Not. model_data%config%atom(i)%identified)) Then
             model_data%reactive_species%indx_new(l) = j
             model_data%config%atom(i)%identified=.True.
             model_data%config%atom(j)%bonds(1)=i
             model_data%config%atom(j)%Nbonds=1
          Else If((model_data%config%atom(j)%element==model_data%reactive_species%bonds%species%type) .And. &
             (.Not. model_data%config%atom(j)%identified)) Then
             model_data%reactive_species%indx_new(l) = i
             model_data%config%atom(j)%identified=.True.
  ...
  Subroutine read_model(files, model_data, frame, ensemble)
    ...
    Else If (model_data%geometry_format%type == 'vasp') Then
  ...
  Subroutine read_input_xyz_format(files, model_data)
    ...
    If (model_data%config%num_atoms /= model_data%reference_composition%numtot) Then
     Write (messages(1),'(3a)') 'ERROR***: Inconsistency between the number of atoms in file ', Trim(input_file),&
                             &' and the amount of atoms defined in &reference_composition. Please check.'   
    ...
    Do While (i < model_data%config%num_atoms)
      If (model_data%reference_composition%N0(j)/=0) Then
        i=i+1
        ...
        If (io/=0) Then
          ...
          Call error_stop(' ')
        Else
          If (Trim(model_data%config%atom(i)%element)/=Trim(model_data%reference_composition%element(j))) Then
            Write (messages(1),'(a,i5,5a)') '***ERROR: Chemical element of atom ', i, &
                                          & ' is set to "', Trim(model_data%config%atom(i)%element), '", but& 
                                          & according to the definition of &reference_composition it must be "',&
                                          & Trim(model_data%reference_composition%element(j)), '".'
            Write (messages(2),'(3a)')  'Check the consistency between the data in ', Trim(input_file),&
                                          & ' and &reference_composition.'
            Call info(messages, 2)
            Call error_stop(' ')
          End If
        End If
        k=k+1
        model_data%config%atom(i)%tag=model_data%reference_composition%tag(j)
        If (k==model_data%reference_composition%N0(j)) Then
          k=0
          j=j+1
        End If
      Else
  ...
  Subroutine read_input_vasp_format(files, model_data, frame, ensemble)
    ...
    If (.Not. model_data%config%allocated_model_geo) Then
      ! Determine vasp elements and amounts of atoms from the data in &reference_composition and &block_species_components
      model_data%config%list%num_elements=1 
      j=1
      model_data%config%list%N0(j)=model_data%reference_composition%N0(j)
      model_data%config%list%element(j)=model_data%reference_composition%element(j)   
      buffer=model_data%reference_composition%element(j)
      Do i = 2,  model_data%reference_composition%atomic_species
        If (Trim(model_data%reference_composition%element(i)) /= Trim(buffer)) Then
          buffer=model_data%reference_composition%element(i)
          If (model_data%reference_composition%N0(i)/=0) Then
            model_data%config%list%num_elements=model_data%config%list%num_elements+1
            j=j+1
            model_data%config%list%N0(j)=model_data%reference_composition%N0(i)
            model_data%config%list%element(j)=model_data%reference_composition%element(i)
          End If
        Else
          model_data%config%list%N0(j)=model_data%config%list%N0(j)+model_data%reference_composition%N0(i)
        End If
      End Do 
    ...
    If (Trim(ensemble)=='npt' .Or. frame==1) Then
      ...
      Do i=1, model_data%config%list%num_elements
        ...
        If (loop) Then
          ...
          Write (messages(3),'(3a)') 'IMPORTANT: The user should also check for inconsistencies between the list/number of atoms&
                                  & in file ', Trim(input_file),' and the settings in &Block_reference_composition.&
      ...
      Write (messages(5),'(3a)') 'Which DO NOT MATCH the order/amount of atoms in file ', Trim(input_file), & 
                              & '. Please review the settings of &reference_composition (or the geometry of the input structure)'
    ...
    Do While (i < model_data%config%num_atoms)
      If (model_data%reference_composition%N0(j)/=0) Then
        ...
        k=k+1
        model_data%config%atom(i)%tag=model_data%reference_composition%tag(j)
        model_data%config%atom(i)%element=model_data%reference_composition%element(j)
        If (k==model_data%reference_composition%N0(j)) Then
          k=0
          j=j+1 
        End If
  ...
  Subroutine check_definition_bonds(model_data, frame)
    ...
    If (model_data%extra_bonds%invoke%fread) Then
      Do k = 1, model_data%extra_bonds%N0
        If (model_data%extra_bonds%bond(k)%value > bond_criterion) Then
          If (frame ==1) Then
            Write (messages(1),'(1x,a,i6)') '***ERROR: problems with the settings of "&extra_reactive_bonds" for the intial&
                                           & configuration.' 
          Else 
            Write (messages(1),'(1x,a,i6,a)') '***ERROR: problems with the settings of "&extra_reactive_bonds" for frame',&
                                          & frame, ' of the trajectory.'
          End If
    ...
    If (model_data%reactive_species%search_envr%cutoff%value > bond_criterion) Then
      If (frame ==1) Then
        Write (messages(1),'(1x,a,i6)') '***ERROR: problems with the settings of "&search_environment" for the intial&
                                       & configuration.' 
      Else 
        Write (messages(1),'(1x,a,i6,a)') '***ERROR: problems with the settings of "&search_environment" for frame',&
                                      & frame, ' of the trajectory.'
      End If
    ...
    If (model_data%reactive_species%bonds%cutoff%value > bond_criterion) Then
      If (frame ==1) Then
  ...
  Subroutine check_model_settings(files, model_data)
    ...
    ! Check "geometry_format"
    If (model_data%geometry_format%fread) Then
      If (model_data%geometry_format%fail) Then
        Write (messages(1),'(2a)')  Trim(error_set), ' Wrong specification for directive "geometry_format"'
        Call info(messages,1)
        Call error_stop(' ') 
      Else
        If (Trim(model_data%geometry_format%type) /= 'xyz' .And. &
            Trim(model_data%geometry_format%type) /= 'vasp') Then
          Write (messages(1),'(2a)') Trim(error_set), ' Valid options for directive "geometry_format":'
      ...
      End If
    Else
      Write (messages(1),'(a,1x,a)')  Trim(error_set), 'Specification of the "geometry_format" directive&
                                     & is required.'
      Call info(messages,1)
      Call error_stop(' ') 
    End If
    
    If (.Not. model_data%reference_composition%invoke%fread) Then
      Write (messages(1),'(2(1x,a))')  Trim(error_set),&
                              & 'Specification of the "&reference_composition" block is missing!'
      Call info(messages,1)
      Call error_stop(' ')
    End If
    ...
    If (model_data%config%cell_units%fread) Then
      If (model_data%config%cell_units%fail) Then
        ...
        Call error_stop(' ') 
      Else
        If (Trim(model_data%geometry_format%type) == 'vasp') Then
           Write (messages(1),'(2a)') Trim(error_set), ' Definition of "cell_units" is incompatible with the&
                                       & option "vasp" for "geometry_format". Please review'
           Call info(messages, 1)
           Call error_stop(' ') 
        End If
        ...
        Else
          ...
          Else If (Trim(model_data%config%cell_units%type) == 'bohr') Then  
            If (Trim(model_data%geometry_format%type) /= 'xyz') Then
              Write (messages(1),'(2a)') Trim(error_set), ' Definition of "cell_units" is incompatible with the&
                                       & choice of "geometry_format". Please review'
             Call info(messages, 1)
             Call error_stop(' ') 
            End If
    ...
    ! Change chemistry
    If (model_data%reactive_chemistry%fread) Then
      If (model_data%reactive_chemistry%fail) Then
        Write (messages(1),'(2(1x,a))') Trim(error_set), 'Missing (or wrong) specification for directive&
                                  & "reactive_chemistry" (choose either .True. or .False.)'
        Call info(messages,1)
        Call error_stop(' ')
      End If
    Else
        Write (messages(1),'(2(1x,a))') Trim(error_set), 'The user must defined the "reactive_chemistry" directive:&
                                      & set to .True. for reactive trajectories.'
        Call info(messages,1)
        Call error_stop(' ')      
    End If
    ...
    If (model_data%reactive_species%invoke%fread) Then
      ...
      Else
        Write (messages(1),'(1x,a)') 'WARNING: the "&reactive_species" block is defined but it is not relevant& 
                                  & to for the analysis of the trajectory, as the user has set the "reactive_chemistry"&
                                  & directive to "False"'
        Call info(messages, 1)         
      End If
    End If
    ...
    If (model_data%reactive_chemistry%stat) Then
      If (.Not. model_data%reactive_species%invoke%fread) Then
          Write (messages(1),'(2a)') Trim(error_set), ' the user has set the "reactive_chemistry" directive to "True"&
                                   &  but the "&reactive_species" block is not defined. Please review settings'
          Call info(messages, 1)
          Call error_stop(' ') 
      End If
    Else
      If (model_data%reactive_species%invoke%fread) Then
          Write (messages(1),'(2a)') Trim(error_set), ' the user has defined the "&reactive_species" block but has not set&
                                   & "reactive_chemistry" to .True. Please change.'
          Call info(messages, 1)
          Call error_stop(' ') 
      End If
    End If    
    ...
    If (model_data%extra_bonds%invoke%fread) Then
      ...
      Else
        Write (messages(1),'(1x,a)') 'WARNING: the "&extra_reactive_bonds" block is defined but it is not relevant&
                                  & to for the analysis of the trajectory, as the user has set the "reactive_chemistry"&
                                  & directive to "False"'
        Call info(messages, 1)
      End If
    ...
    If (model_data%nonreactive_species%invoke%fread) Then
      Call check_definition_non_reactive_species(files, model_data)
    End If
    
    ! Set the flag to later allocation of model related arrays
  ...
  Subroutine check_chemistry_criteria(files, model_data)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Subroutine to check the format and directives of the &reactive_species block
    !
    ! author    - i.scivetti July 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(file_type),     Intent(InOut) :: files(:)
    ...
    error_set = '***ERROR in file '//Trim(files(FILE_SET)%filename)//' -'
    Write (messages(1),'(1x,2a)')  Trim(error_set), ' "&reactive_species" block'

    If (model_data%reactive_species%N0%fread) Then
      If (model_data%reactive_species%N0%fail) Then
    ...
    ! Type of reactive sute: donor or acceptor
    If (model_data%reactive_species%type%fread) Then
      If (model_data%reactive_species%type%fail) Then
        Write (messages(2),'(1x,a)')  'Missing or wrong specification for directive "type"'
        Call info(messages,2)
        Call error_stop(' ') 
      Else
        If (Trim(model_data%reactive_species%type%type) /= 'donor'    .And. &
            Trim(model_data%reactive_species%type%type) /= 'acceptor') Then
          Write (messages(2),'(1x,a)') 'Wrong setting for directive "type". Valid options&
                                       & are "donor" or "acceptor"'
          Call info(messages,2)
          Call error_stop(' ') 
        End If
      End If
    Else
      Write (messages(2),'(1x,a)')  'The "type" directive has not been defined. It must set&
                                  & to either "donor" or "acceptor"'
      Call info(messages, 2)
      Call error_stop(' ') 
    End If    

    If (model_data%reactive_species%bonds%criteria%fread) Then
    ...
    If (model_data%reactive_species%search_envr%criteria%fread) Then
      Call check_search_environment(files, model_data) 
    Else
  ...
  Subroutine check_search_environment(files, model_data)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    ! environment cutoff
    If (.Not. model_data%reactive_species%search_envr%cutoff%fread) Then
      model_data%reactive_species%search_envr%cutoff%tag='cutoff'
    End If
    Call check_length_directive(model_data%reactive_species%search_envr%cutoff, messages(1), .True., 'directive')

    ! Exclude pairs
    If (model_data%reactive_species%search_envr%info_exclude%fread) Then
      If (model_data%reactive_species%search_envr%info_exclude%fail) Then    
        Write (messages(2),'(1x,a)')  'Wrong specification for directive "exclude_pairs". See the "use_code.md" file.'
        Call info(messages, 2)
        Call error_stop(' ') 
      End If

      If (model_data%reactive_species%search_envr%N0_excl < 1) Then
        Write (messages(2),'(1x,a)')  'The number of "exclude_pairs" must be positive'
        Call info(messages, 2)
        Call error_stop(' ') 
      End If
      
      Do k=1, model_data%reactive_species%search_envr%N0_excl
        found=.False.
        Do j=1, model_data%reference_composition%atomic_species
          If (Trim(model_data%reactive_species%search_envr%tg_excl(k))== Trim(model_data%reference_composition%tag(j))) Then
            found=.True.
          End If
        End Do
        If (.Not. found) Then
          Write (messages(2),'(3(1x,a))') 'The tag "', Trim(model_data%reactive_species%search_envr%tg_excl(k)), &
                                      & '" in the "exclude_pairs" directive is not defined as a tag in the&
                                      & "&reference_composition" block. Please review'
          Call info(messages, 2)
          Call error_stop(' ') 
        End If
      End Do

      Do j=1, model_data%reactive_species%search_envr%N0_excl-1
        Do k=j+1, model_data%reactive_species%search_envr%N0_excl
          If (Trim(model_data%reactive_species%search_envr%tg_excl(j))==&
              Trim(model_data%reactive_species%search_envr%tg_excl(k))) Then
            Write (messages(2),'(3(1x,a))') 'Tag', Trim(model_data%reactive_species%search_envr%tg_excl(j)),&
    ...
    If (model_data%reactive_species%search_envr%info_include%fread) Then
      If (model_data%reactive_species%search_envr%info_include%fail) Then    
      ...
      Do k=1, model_data%reactive_species%search_envr%N0_incl
        found=.False.
        Do j=1, model_data%reference_composition%atomic_species
          If (Trim(model_data%reactive_species%search_envr%tg_incl(k))== Trim(model_data%reference_composition%tag(j))) Then
            found=.True.
          End If
        End Do
        If (.Not. found) Then
          Write (messages(2),'(3(1x,a))') 'The tag "', Trim(model_data%reactive_species%search_envr%tg_incl(k)), &
      ...
      Do j=1, model_data%reactive_species%search_envr%N0_incl-1
       Do k=j+1, model_data%reactive_species%search_envr%N0_incl
         If (Trim(model_data%reactive_species%search_envr%tg_incl(j))==&
             Trim(model_data%reactive_species%search_envr%tg_incl(k))) Then
           Write (messages(2),'(3(1x,a))') 'Tag', Trim(model_data%reactive_species%search_envr%tg_incl(j)),&
                                         &'is repeated in the list!'
    ...
    If (model_data%reactive_species%search_envr%info_exclude%fread .And.&
       model_data%reactive_species%search_envr%info_include%fread) Then
      Do k=1, model_data%reactive_species%search_envr%N0_excl
        found=.False.
        Do j=1, model_data%reactive_species%search_envr%N0_incl
          If (model_data%reactive_species%search_envr%tg_excl(k)==&
              model_data%reactive_species%search_envr%tg_incl(j)) Then
            found=.True.
          End If
        End Do
        If (.Not. found) Then
    ...
    If (model_data%reactive_species%search_envr%info_exclude%fread .And.&
        model_data%reactive_species%search_envr%info_exclude%fread) Then
      If (model_data%reactive_species%search_envr%N0_excl==model_data%reactive_species%search_envr%N0_incl .And.&
          model_data%reactive_species%search_envr%N0_excl==1) Then
    ...
    model_data%reactive_species%search_envr%check=.True.

  End Subroutine check_search_environment
  ...
  Subroutine check_bonding_criteria(files, model_data)
    ...
    If (model_data%reactive_species%bonds%species%fread) Then
      If (model_data%reactive_species%bonds%species%fail) Then
      ...
      found=.False.
      Do j=1, model_data%reference_composition%atomic_species
        If (Trim(model_data%reactive_species%bonds%species%type)== Trim(model_data%reference_composition%element(j))) Then
          found=.True.
        End If
      End Do
      If (.Not. found) Then
        Write (messages(2),'(3(1x,a))') 'The definition of the "only_element" directive is "',&
                                      & Trim(model_data%reactive_species%bonds%species%type), &
                                      & '", which is not defined as a valid CHEMICAL ELEMENT&
                                      & in the "&reference_composition" block. Please review'
        Call info(messages, 2)
        Call error_stop(' ') 
      End If
    Else
    ...
    ! Number of bonds 
    If (model_data%reactive_species%bonds%N0%fread) Then
      If (model_data%reactive_species%bonds%N0%fail) Then
        Write (messages(2),'(1x,a)')  'Wrong specification for directive "number_of_bonds"'
        Call info(messages,2)
        Call error_stop(' ') 
      Else
        If (model_data%reactive_species%bonds%N0%value < 1) Then
    ...
    ! Bond cutoff
    If (.Not. model_data%reactive_species%bonds%cutoff%fread) Then
      model_data%reactive_species%bonds%cutoff%tag='cutoff'
    End If
    Call check_length_directive(model_data%reactive_species%bonds%cutoff, messages(1), .True., 'directive')
    
  End Subroutine check_bonding_criteria
  
  Subroutine check_extra_bonding(files, model_data)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Subroutine to check the format and directives the &extra_reactive_bonds block
    !
    ! author    - i.scivetti July 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(file_type),     Intent(InOut) :: files(:)
    ...
    error_set = '***ERROR in file '//Trim(files(FILE_SET)%filename)//' - ' 
    Write (messages(1),'(a)')  Trim(error_set)//'Wrong specification in the &extra_reactive_bonds" block.'
    
    Do k = 1, model_data%extra_bonds%N0
      ! Loop over tg1
      found = .False.
      Do j = 1, model_data%reference_composition%atomic_species
        If (Trim(model_data%extra_bonds%tg1(k)) == Trim(model_data%reference_composition%tag(j))) Then
          tg1_element=Trim(model_data%reference_composition%element(j))
          found = .True.   
        End If
      End Do
      ...
      If (.Not. found) Then      
        Write (messages(2),'(1x,3a,i3,a)') 'Species "', Trim(model_data%extra_bonds%tg1(k)), '" defined for the first tag& 
                                          & of the type of bond ', k, ' has NOT been defined in the &reference_composition&
      ...
      found = .False.
      Do j = 1, model_data%reference_composition%atomic_species
        If (Trim(model_data%extra_bonds%tg2(k)) == Trim(model_data%reference_composition%tag(j))) Then
          tg2_element=Trim(model_data%reference_composition%element(j))
          found = .True.   
        End If
      End Do
    ...
    ! Check intrinsic tags wihtin the bond
      ...
      If (Trim(tg1_element) /= Trim(model_data%reactive_species%bonds%species%type) .And. &
          Trim(tg2_element) /= Trim(model_data%reactive_species%bonds%species%type)) Then
      ...
      found=.False. 
      Do j = 1, model_data%reactive_species%search_envr%N0_incl 
        If ((Trim(model_data%reactive_species%search_envr%tg_incl(j)) == Trim(model_data%extra_bonds%tg1(k))) .Or. &
            (Trim(model_data%reactive_species%search_envr%tg_incl(j)) == Trim(model_data%extra_bonds%tg2(k)))) Then
            found=.True.
        End If
      End Do
      ...
      If (.Not. found) Then      
          Write (messages(2),'(1x,a,i3,5a)') 'None of the two species tags of the type of bond ', k, &
                                          ...
                                          & ') is defined in the "include_tags" directive of the "&search_environment"&
  ...
  Subroutine check_definition_non_reactive_species(files, model_data)
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! Subroutine to check the format and directives of the 
    ! &selected_nonreactive_species block
    !
    ! author    - i.scivetti May 2025
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    Type(file_type),     Intent(InOut) :: files(:)
    ...
    If (model_data%nonreactive_species%atomic_components%fread) Then
      ! Check information within the &atomic_components sub-block
      Do k = 1, model_data%nonreactive_species%num_components
        ! Loop over elements
        found = .False.
        Do j = 1, model_data%reference_composition%atomic_species
          If (Trim(model_data%nonreactive_species%element(k)) == Trim(model_data%reference_composition%element(j))) Then
            found = .True.   
          End If
        End Do
        ...
        If (.Not. found) Then      
          Write (messages(2),'(1x,3a,i3,a)') '(Sub-block &atomic_components): Atomic element "', &
                                       &Trim(model_data%nonreactive_species%element(k)), &
                                       &'" defined for the ',& 
                                       & k, ' component has NOT been defined in the &reference_composition&
                                       & block. Please review the settings'
          Call info(messages, 2)         
          Call error_stop(' ')
        End If
  
        If (model_data%nonreactive_species%N0_element(k)<1) Then
          Write (messages(2),'(1x,3a,i3,a)') '(Sub-block &atomic_components): Atomic element "',&  
                                      & Trim(model_data%nonreactive_species%element(k)),&
                                      & '" defined for the ',&
        ...
        ElseIf (model_data%nonreactive_species%num_components>max_components) Then
     ...
     Do k = 1, model_data%nonreactive_species%num_components
       Do j=1, model_data%nonreactive_species%num_components
         If (k /= j) Then
           If (Trim(model_data%nonreactive_species%element(k))==Trim(model_data%nonreactive_species%element(j))) Then
             Write (messages(2),'(1x,a)') 'Problems in "&atomic_components": definition for the atomic element "'& 
                                         &//Trim(model_data%nonreactive_species%element(k))//'" is repeated.'
             Call info(messages, 2)
             Call error_stop(' ') 
           End If 
         End If
     ...
     ! Calculate total number of atoms per species
      model_data%nonreactive_species%atoms_per_species=0
      Do j=1, model_data%nonreactive_species%num_components
        model_data%nonreactive_species%atoms_per_species = model_data%nonreactive_species%atoms_per_species + &
                                                          model_data%nonreactive_species%N0_element(j)
      End Do
      
      If (model_data%nonreactive_species%atoms_per_species>max_at_species) Then
          Write (messages(2),'(1x,a,i2,a)')  'The &atomic_components block indicates that each species has ',&
                                   & model_data%nonreactive_species%atoms_per_species, ' atoms. Are you sure&
                                   & you want to monitor nonreactive species with such a large number of atoms?'
         Call info(messages, 2)
         Call error_stop(' ') 
      End If
      ...
      If (model_data%nonreactive_species%atoms_per_species == 1) Then
          Write (messages(2),'(1x,a,i2,a)')  'The &atomic_components block indicates that each species has ',&
                                   & model_data%nonreactive_species%atoms_per_species, ' atoms. This block is&
      ...
      Do j=1, model_data%nonreactive_species%num_components
        If (Trim(ref_element)==Trim(model_data%nonreactive_species%element(j)))  Then
           If (model_data%nonreactive_species%N0_element(j) /= 1) Then
      ...
      End Do
    Else
      model_data%nonreactive_species%atoms_per_species=1
    End If
   
    If (.Not. model_data%nonreactive_species%name%fread) Then
      If (model_data%nonreactive_species%atoms_per_species /= 1 ) Then
        Write (messages(2),'(1x,a)')  'The "name" directive has not been defined.'
        Call info(messages, 2)
        Call error_stop(' ') 
      Else
        model_data%nonreactive_species%name%type=Trim(model_data%nonreactive_species%reference_tag%type)
      End If
    Else
      If (model_data%nonreactive_species%atoms_per_species == 1 ) Then
    ...
    If (.Not. model_data%nonreactive_species%reference_tag%fread) Then
    ...
    Else
      ...
      Do While (j <= model_data%reference_composition%atomic_species .And. flag)
        If (Trim(model_data%reference_composition%tag(j))==Trim(model_data%nonreactive_species%reference_tag%type)) Then
          flag=.False.
          ref_element=Trim(model_data%reference_composition%element(j))
        End If  
        j=j+1
      End Do
      If (flag) Then
        Write (messages(2),'(1x,a)')  'The setting for "reference_tag" does not match any of&
                                    & tags defined in the "&reference_composition" block'
        Call info(messages, 2)
        Call error_stop(' ') 
      End If 
    End If
    ...
    If (.Not. model_data%nonreactive_species%bond_cutoff%fread) Then
      model_data%nonreactive_species%bond_cutoff%tag='bond_cutoff'
    End If
    Call check_length_directive(model_data%nonreactive_species%bond_cutoff, messages(1), .True., 'directive')
    
    If (model_data%nonreactive_species%compute_amount%fread) Then    
      If (model_data%nonreactive_species%compute_amount%fail) Then   
        Write (messages(2),'(1x,a)')  'Wrong or missing definition for "compute_amount" directive.'
        Call info(messages, 2)
        Call error_stop(' ') 
      End If
    Else
      model_data%nonreactive_species%compute_amount%stat=.False.
    End If
  ...
  End Subroutine check_definition_non_reactive_species
  ...
  Subroutine print_model_settings(files, model_data) 
    ...
    Character(Len=256) :: messages(4), word, N0chem, nbonds, ncutoff, anti_species
    Integer(Kind=wi)   :: j

    Call info(' ', 1) 
    Call info('Model settings', 1) 
    Call info('==============', 1) 
    Write (word,*) model_data%reference_composition%numtot
    Write (messages(1),'(1x,a)') '- the atomic model contains '//Trim(Adjustl(word))//' atoms.&    
                                 & The reference tag and element for each atomic species is defined&
                                 & in the "&reference_composition" block.'
    ...
    If (model_data%reactive_chemistry%stat) Then
      Write (messages(1),'(1x,a)') '- the chemical composition of species is expected to change&
                                    & along the trajectory (reactive system). The reactive species are&
                                    & defined via the "&reactive_species" block'
    Else
      Write (messages(1),'(1x,a)') '- the chemical composition of all constituent species is set to remain&
                                    & unaltered along the trajectory (non-reactive system)'
    End If
    Call info(messages, 1)
    
    If (model_data%nonreactive_species%invoke%fread) Then
      If (model_data%reactive_species%invoke%fread) Then
         Write (messages(1),'(1x,a)') '- the "&selected_nonreactive_species" block defines the composition of&
                                 & selected, non-reactive species that are part of the reactive system'
      Else
         Write (messages(1),'(1x,a)') '- the "&selected_nonreactive_species" block defines the composition of&
                                 & selected species that are part of system'
      End If
      Call info(messages, 1)
    End If

    If (model_data%reactive_chemistry%stat) Then
      If (Trim(model_data%reactive_species%type%type) == 'donor') Then
        anti_species='acceptor'
      Else If (Trim(model_data%reactive_species%type%type) == 'acceptor') Then
        anti_species='donor'
      End If
      Write (N0chem,*) model_data%reactive_species%N0%value
      Write (nbonds,*) model_data%reactive_species%bonds%N0%value
      Write (ncutoff,'(f10.2)') model_data%reactive_species%bonds%cutoff%value
      
      Write (messages(1),'(1x,a)') 'The procedure is set to identify and track '//Trim(Adjustl(N0chem))//' "'//& 
                            &Trim(model_data%reactive_species%type%type)//'" species along the trajectory such that they contain:'
      If (model_data%reactive_species%search_envr%N0_incl > 1) Then                      
        Write (messages(2),'(1x,a,(*(3x,a)))') '- any of the following reference atomic tags: ',&
                                    & (Trim(model_data%reactive_species%search_envr%tg_incl(j)),&
                                    & j = 1, model_data%reactive_species%search_envr%N0_incl)
      Else
        Write (messages(2),'(1x,a,(*(3x,a)))') '- "'//Trim(model_data%reactive_species%search_envr%tg_incl(1))//&
                                              &'" as the reference atomic tag'
      End If
      
      Write (messages(3),'(1x,a)') '- a maximum of '//Trim(Adjustl(nbonds))//&
                                  &' bonds with "'//Trim(model_data%reactive_species%bonds%species%type)//&
                                  & '" atoms. A bond is formed when the interactomic distance (cutoff) is&
                                  & lower than '//Trim(Adjustl(ncutoff))//' Angstroms'
      Call info(' ', 1)
      Call info(messages, 3)
      
      If (model_data%extra_bonds%invoke%fread) Then
        Write (messages(1),'(1x,a)') 'From the definitions in "&extra_reactive_bonds" (within the &bonding_criteria block),&
      ...
      Write (messages(1),'(1x,a)') 'Atomic species that are part of "'//Trim(model_data%reactive_species%type%type)//'s"&
                                   & will be identified with an extra "*" symbol added to their reference atomic tags'
      Write (ncutoff,'(f10.2)') model_data%reactive_species%search_envr%cutoff%value                             
      Write (messages(2),'(1x,a)') 'The algorithm monitors bond breaking and formation by considering "'&
                                   &//Trim(anti_species)//'" species within a cutoff radius of '//Trim(Adjustl(ncutoff))//&
                                   &' Angstroms around each "'//Trim(model_data%reactive_species%type%type)//'".'
      Write (messages(3),'(1x,a)') 'The search for new "'//Trim(model_data%reactive_species%type%type)//'" sites...'
      Write (messages(4),'(1x,a,(*(3x,a)))') '- is restricted to the atomic tag(s): ',&
                                    & (Trim(model_data%reactive_species%search_envr%tg_incl(j)),&
                                    & j = 1, model_data%reactive_species%search_envr%N0_incl)
      Call info(messages, 4)
      If (model_data%reactive_species%search_envr%info_exclude%fread) Then
        Write (messages(1),'(1x,a,(*(3x,a)))') '- excludes the donor-acceptor pairs(s): ', &
         &  (Trim(model_data%reactive_species%search_envr%tg_excl(j))//'*--'&
         &//Trim(model_data%reactive_species%search_envr%tg_excl(j)), j = 1, model_data%reactive_species%search_envr%N0_excl)
        Call info(messages, 1)
      End If
    End If
